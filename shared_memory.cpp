...
//   запускать numa_run_on_node(). Это сейчас проверяю (?).
...
// - проверить работу с numa
//   есть интересная статья, посмотреть как прошло разделение памяти по numa:
//   https://codywu2010.wordpress.com/2015/10/01/how-shared-library-gets-mmaped-on-numa-system/
